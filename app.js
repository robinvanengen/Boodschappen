...
const initial = {
  recipes:[], plans:{}, extras:[], checked:{}, hidden:{}, dayDone:{}, quickMeals:{}, mealMeta:{}, storeOverrides:{}, temporaryRecipes:{}, customMealItems:{}, version:5
};
...
function openMealEditor(date,mealIndex){
  ...
  const ingredients=recipe.ingredients.map((x,index)=>makeIngredient(x,`${date}-${mealIndex}-${recipe.id}-${index}`)).join('')+(data.customMealItems?.[`${date}-${mealIndex}`]||[]).map(x=>makeIngredient([x.name,x.amount,x.store],x.id)).join('');
